...
##                        self.Log.WriteLog('error', self.LogText)
        ...
        def __init__(self):
    		...
    		self.EMAIL_SENDER="jira@jira.corp.aryaka.com"
    		self.EMAIL_SUBJECT="LIVE REPORT"
    		self.EMAIL_RECIPIENTS=["venkatesh@aryaka.com"]
...
##          f="/root/bkp/ASN_Report_Scripts/logfiles.tgz"
         #     f="/root/bkp/ASN_Report_Scripts/"+f
          ...
          #msg.attach(part)
  	  #server.login('rmb-jira', 'Aryaka#345')

          smtp = smtplib.SMTP(self.EMAIL_SMTP_SERVER, self.EMAIL_SMTP_PORT)
          smtp.set_debuglevel(True)
	  smtp.sendmail(self.EMAIL_SENDER, self.EMAIL_RECIPIENTS, msg.as_string())
          smtp.close()
